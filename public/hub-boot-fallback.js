    ...
    el.innerHTML =
      ...
      (detail
        ? '<pre style="margin:0 0 1rem;padding:0.75rem;border-radius:0.5rem;background:rgba(15,23,42,0.85);color:#fca5a5;font:11px/1.45 ui-monospace,Consolas,monospace;text-align:left;white-space:pre-wrap;word-break:break-word;max-height:10rem;overflow:auto">' +
          ...
          "</pre>"
        : "") +
      '<p style="margin:0 0 1rem;font-size:0.7rem;color:#64748b">Try hard refresh <kbd style="padding:0.1rem 0.35rem;border-radius:0.25rem;background:rgba(99,102,241,0.2);color:#a5b4fc">Ctrl+Shift+R</kbd> or restart Vite with <code>pnpm desktop:open --dev</code> (keeps Vite warm).</p>' +
      '<div style="display:flex;gap:0.5rem;justify-content:center;flex-wrap:wrap">' +
      '<button type="button" id="hub-boot-reload" style="cursor:pointer;border:1px solid rgba(129,140,248,0.4);border-radius:0.5rem;background:rgba(99,102,241,0.15);color:#c7d2fe;padding:0.45rem 1rem;font-size:0.75rem">Retry</button>' +
      '<button type="button" id="hub-boot-devtools" style="cursor:pointer;border:1px solid rgba(148,163,184,0.35);border-radius:0.5rem;background:rgba(15,23,42,0.6);color:#e2e8f0;padding:0.45rem 1rem;font-size:0.75rem">Open DevTools</button>' +
      "</div>" +
      "</div>";
    var reloadBtn = el.querySelector("#hub-boot-reload");
    if (reloadBtn) reloadBtn.onclick = function () { location.reload(); };
    var toolsBtn = el.querySelector("#hub-boot-devtools");
    if (toolsBtn) {
      toolsBtn.onclick = function () {
        try {
          if (window.autovideo && typeof window.autovideo.openDevTools === "function") {
            window.autovideo.openDevTools();
            return;
          }
        } catch (e) {}
      };
      if (!(window.autovideo && window.autovideo.openDevTools)) {
        toolsBtn.title = "View → Toggle Developer Tools (Ctrl+Shift+I)";
      }
    }
  }
