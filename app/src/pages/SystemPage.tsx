...
export default function SystemPage() {
  ...
  const tab: SystemTab = searchParams.get('stab') === 'template' ? 'template' : 'overview';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        <SystemTabLink to="/system" active={tab === 'overview'} icon={<LayoutDashboard size={15} />}>
          Overview
        </SystemTabLink>
        <SystemTabLink to="/system?stab=template" active={tab === 'template'} icon={<Palette size={15} />}>
          Design Template
        </SystemTabLink>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-white/10 bg-[var(--panel)] px-5 py-4 text-xs text-[var(--muted)]">
        Workspace preferences (system stats refresh, etc.) are in the sidebar footer → <strong className="text-amber-200/90">Settings</strong>.
      </section>
