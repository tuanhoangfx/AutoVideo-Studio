...
import {
  readKeyframeSceneDirectoryColumnsForDraft,
  restoreKeyframeSceneDirectoryColumnsFromDraft,
  KEYFRAME_SCENE_DIRECTORY_COLUMNS_CHANGE,
} from '@/lib/keyframe-scene-directory-prefs';
import { ProjectTabs } from '@/components/studio/ProjectTabs';
import { ScriptPanel } from '@/components/studio/ScriptPanel';
import { AudioPreview } from '@/components/studio/AudioPreview';
import { ImageLibrary, type LibraryImage, type LibraryImageInput } from '@/components/studio/ImageLibrary';
...
const KeyframeTimeline = lazy(() =>
  import('@/components/studio/KeyframeTimeline').then((m) => ({ default: m.KeyframeTimeline })),
);
import {
...
import { sceneExportStatusAtIndex } from '@/lib/keyframe-scene-export-skip';
...
import { supportsDesktopSilentSave } from '@/lib/studio-download-target';
import { computeSceneDurationsSec, scriptHoldTailSec, totalExportDurationSec } from '@/lib/export-duration';
...
import { buildSceneLines, moveItem, sceneLinesEqual, shuffleArray, sourceFolderName } from '@/lib/studio/studio-scene-utils';
import type { StudioAspect, StudioDownloadRecord } from '@/lib/studio/studio-types';
import {
  PanelHead,
  PreviewExportStatus,
...
import {
  ScriptPanelHead,
} from '@/components/studio/ScriptPanel';
import { useStudioJobKpis } from '@/hooks/use-studio-job-kpis';
import {
  filterJobsByStudioStatus,
  toggleStudioJobStatusFilter,
  type StudioJobStatusFilterKey,
} from '@/lib/studio/studio-job-status-filter';
import { StudioHubChrome } from '@/components/workspace/StudioHubChrome';
...
export default function StudioPage() {
  ...
  const [sceneOrderMode, setSceneOrderMode] = useState<SceneOrderMode>('sequential');
  const [keyframeDirectoryColumns, setKeyframeDirectoryColumns] = useState<string[]>(() =>
    readKeyframeSceneDirectoryColumnsForDraft(),
  );

  useEffect(() => {
    const sync = () => setKeyframeDirectoryColumns(readKeyframeSceneDirectoryColumnsForDraft());
  ...
  const setStudioVoice = useCallback((next: string) => {
    setVoice(resolveStudioVoiceId(next));
  }, []);
  ...
  const applyEditorSnapshot = useCallback((snap: EditorSnapshot) => {
    ...
    setVoice(resolveStudioVoiceId(snap.voice));
    ...
    setSceneOrderMode(snap.sceneOrderMode ?? 'sequential');
    if (snap.keyframeDirectoryColumns?.length) {
      restoreKeyframeSceneDirectoryColumnsFromDraft(snap.keyframeDirectoryColumns);
      setKeyframeDirectoryColumns(readKeyframeSceneDirectoryColumnsForDraft());
    }
  }, []);
    ...
    (async () => {
      try {
        ...
        if (draft) {
          ...
          setVoice(resolveStudioVoiceId(draft.voice));
          setRate(draft.rate);
          ...
          if (draft.sceneOrderMode) setSceneOrderMode(draft.sceneOrderMode);
          restoreKeyframeSceneDirectoryColumnsFromDraft(draft.keyframeDirectoryColumns);
          setKeyframeDirectoryColumns(readKeyframeSceneDirectoryColumnsForDraft());
        }
        setImages(libImages);
        ...
        editorCacheRef.current.set(draftId, {
          ...
          downloadState: 'idle',
          downloadMessage: '',
          sceneOrderMode: draft?.sceneOrderMode ?? 'sequential',
          keyframeDirectoryColumns: draft?.keyframeDirectoryColumns ?? readKeyframeSceneDirectoryColumnsForDraft(),
        });
      } catch {
  ...
  const draftState: DraftState = useMemo(
    ...
    [topic, lines, voice, rate, ttsProvider, aspect, fps, resolution, videoQuality, outputFormat, autoDownload, downloadDirectoryName, bgmVolume, subtitleStyle, images.length, sceneOrderMode, keyframeDirectoryColumns]
  );
  ...
  const changeTranscript = (i: number, text: string) =>
  ...
  const shuffleScenes = useCallback((indexes: number[]) => {
    setLines((prevLines) => {
      if (prevLines.length <= 1) return prevLines;
      const fullShuffle = indexes.length === 0 || indexes.length >= prevLines.length;
      const targets = fullShuffle
        ? prevLines.map((_, index) => index)
        : [...indexes].sort((a, b) => a - b);
      if (targets.length <= 1) return prevLines;

      let nextLines: typeof prevLines;
      if (fullShuffle) {
        nextLines = shuffleArray(prevLines);
      } else {
        const picked = targets.map((index) => prevLines[index]);
        const shuffled = shuffleArray(picked);
        nextLines = [...prevLines];
        targets.forEach((sceneIndex, order) => {
          nextLines[sceneIndex] = shuffled[order];
        });
      }

      setSelectedImageIndexes(nextLines.map((line) => line.image_index));
      setSelectedScene(0);
      return nextLines;
    });
  }, []);
  ...
  useEffect(() => {
    setSelectedImageIndexes((prev) => {
      const next = prev.filter((idx) => idx >= 0 && idx < images.length);
  ...
  useEffect(() => {
    setLines((prev) => {
      const next = buildSceneLines(selectedImageIndexes, prev, imageDurationSec);
      return sceneLinesEqual(prev, next) ? prev : next;
    });
    setSelectedScene((prev) => {
      const clamped = Math.max(0, Math.min(prev, Math.max(0, selectedImageIndexes.length - 1)));
      return clamped === prev ? prev : clamped;
    });
  }, [imageDurationSec, selectedImageIndexes]);

  useEffect(() => {
    const next = selectedImageIndexes[selectedScene] ?? selectedImageIndexes[0] ?? 0;
    setSelectedImage((prev) => (prev === next ? prev : next));
  ...
  const workerXfadeAvailable = workerCompose?.xfadeAvailable ?? false;
  const sceneDurationsSec = useMemo(
    () =>
      computeSceneDurationsSec(
        ...
        workerXfadeAvailable
      ),
    [exportDurationMode, renderLines, imageDurationSec, transcriptDurationSec, workerXfadeAvailable]
  );
  const holdTailSec = useMemo(
    () =>
      scriptHoldTailSec(
        exportDurationMode,
        sceneDurationsSec,
    ...
    [exportDurationMode, sceneDurationsSec, transcriptDurationSec, renderLines, workerXfadeAvailable]
  );
  ...
  useEffect(() => {
    if (!jobStatusFilter || !activeJobId) return;
    const visible = filterJobsByStudioStatus(jobs, jobStatusFilter);
    if (visible.some((job) => job.id === activeJobId)) return;
    const next = visible[0];
    if (next) switchJob(next.id);
    else {
      setActiveJobId(null);
      applyEmptyEditor();
    }
  }, [jobStatusFilter, jobs, activeJobId, switchJob, setActiveJobId, applyEmptyEditor]);
  ...
  const sequenceScenes = useMemo(() => {
    const filtered = lines.filter((l) => images[l.image_index]);
    const editorDurations = filtered.map((l) => l.durationSec ?? imageDurationSec);
    const editorStarts = editorDurations.map((_, i) =>
      editorDurations.slice(0, i).reduce((a, b) => a + b, 0),
    );
    return filtered
      .map((l, order) => ({
        text: l.text,
        ...
        effect: l.effect,
        durationSec: sceneDurationsSec[order] ?? l.durationSec ?? imageDurationSec,
        transition: normalizeTransitionForWorker(l.transition),
        exportStatus: sceneExportStatusAtIndex(
          order,
          editorStarts,
          editorDurations,
          exportDurationMode,
      ...
      .filter((scene) => (scene.durationSec ?? 0) > 0);
  }, [
    lines,
    images,
    imageDurationSec,
  ...
  const hideManualDownloadLink =
    hydrated && supportsDesktopSilentSave() && autoDownload && Boolean(downloadDirectoryName);
  ...
  return (
    <StudioHubChrome>
      ...
      {/* TOP: Project tabs + job KPI badges */}
      <div className="hub-card mb-2 overflow-hidden">
        <div className="flex items-stretch gap-0">
          <div className="min-w-0 flex-1 overflow-x-auto studio-timeline-scroll">
            <ProjectTabs
              jobs={jobs}
              ...
              onToggleStatusFilter={toggleJobStatusFilter}
              onSelect={switchJob}
              onNew={newProject}
      ...
      <div className="grid h-[50vh] min-h-[24rem] w-full min-w-0 grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(17rem,0.85fr)] items-stretch gap-2">
        ...
        <section className="flex min-h-0 flex-col gap-2">
          <div className="hub-card flex h-[15rem] shrink-0 flex-col overflow-hidden">
            <PanelHead icon={<PlayCircle size={13} />} title="Preview" />
            ...
            durationMismatchMs > 2000 ? (
              <div className="flex items-center justify-between gap-2 border-b border-rose-400/30 bg-rose-500/15 px-3 py-1.5 text-[11px] text-rose-100">
                <span>
                  Output length differs from target by{' '}
                  {Math.round(durationMismatchMs / 1000)}s (expected{' '}
                  {formatDuration((currentJob.expected_duration_ms ?? 0) / 1000)}, got{' '}
                  {currentJob.output_duration_ms != null
                    ? formatDuration(currentJob.output_duration_ms / 1000)
                    : '—'}
                  ). Re-export after worker restart if this persists.
                </span>
                <button
                  type="button"
            ...
            {previewMode === 'sequence' && canPreview ? (
              <SequencePreview
                key={`${exportDurationMode}-${transcriptDurationSec.toFixed(1)}-${totalVideoSec.toFixed(1)}-${sequenceScenes.length}`}
          ...
          {/* Script card */}
          <div className="hub-card flex min-h-0 flex-1 flex-col overflow-hidden">
            <ScriptPanelHead
      ...
      <div className="mt-3 min-h-0 shrink-0">
        ...
        <KeyframeTimeline
          ...
          onShuffleScenes={shuffleScenes}
          sceneOrderMode={sceneOrderMode}
          onSceneOrderModeChange={setSceneOrderMode}
          imageDurationSec={imageDurationSec}
          onImageDurationSec={setImageDurationSec}
          exportDurationSec={totalVideoSec}
          ...
          onPlayheadSec={setPreviewPlayhead}
          waveforms={sequenceTiming?.waveforms}
          sceneDurationsSec={sceneDurationsSec}
          holdTailSec={holdTailSec}
        />
        </Suspense>
      </div>
